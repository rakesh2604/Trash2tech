...
export function IncentivesFilters(props: { current: { actorType?: string; status?: string } }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-4 mb-6">
      <div>
        <label htmlFor="inc-filter-actor" className="block text-xs font-medium text-white/70 mb-1">Actor type</label>
        <select id="inc-filter-actor" className="input-glass w-auto min-w-[160px]" value={actorType} onChange={(e) => update('actorType', e.target.value)}>
          {ACTOR_TYPES.map((o) => <option key={o.value || '_'} value={o.value}>{o.label}</option>)}
        </select>
      </div>
      <div>
        <label htmlFor="inc-filter-status" className="block text-xs font-medium text-white/70 mb-1">Status</label>
        <select id="inc-filter-status" className="input-glass w-auto min-w-[140px]" value={status} onChange={(e) => update('status', e.target.value)}>
