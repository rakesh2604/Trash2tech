  ...
  if (loading) {
    return (
      <AppShell title="E-waste Console">
        <h2 className="text-xl font-bold tracking-tight text-white">Create pickup</h2>
        <p className="text-sm text-white/70 mt-1">Loading hubs and categories…</p>
      </AppShell>
    );
  }
    ...
    <AppShell title="E-waste Console">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 max-w-xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-white/80 mb-1">Source channel</label>
            <select
              className="input-glass min-h-[44px]"
          ...
          <div>
            <label className="block text-sm font-medium text-white/80 mb-1">Citizen type</label>
            <select
              className="input-glass min-h-[44px]"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-white/80 mb-1">Citizen name</label>
            <input
          ...
          <div>
            <label className="block text-sm font-medium text-white/80 mb-1">Citizen phone</label>
            <input
              className="input-glass"
        ...
        <div>
          <label className="block text-sm font-medium text-white/80 mb-1">Address line 1 *</label>
          <input
            required
            className="input-glass"
            value={form.line1}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-white/80 mb-1">Line 2</label>
            <input
              className="input-glass"
              value={form.line2}
          ...
          <div>
            <label className="block text-sm font-medium text-white/80 mb-1">Landmark</label>
            <input
              className="input-glass"
              value={form.landmark}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-white/80 mb-1">Pincode *</label>
            <input
              required
              className="input-glass"
              value={form.pincode}
        ...
        {submitStatus.type === 'success' && (
          <p className="text-sm text-eco bg-eco/10 rounded-xl px-3 py-2 border border-eco/30">{submitStatus.message}</p>
        )}
        {submitStatus.type === 'error' && (
          <p className="text-sm text-red-300 bg-red-500/10 rounded-xl px-3 py-2 border border-red-500/30">{submitStatus.message}</p>
        )}
        <div className="flex flex-wrap gap-3 pt-2">
          <button type="submit" className="btn-glass-primary">
            Create pickup
          </button>
          <Link href="/admin/pickups" className="inline-flex min-h-[44px] items-center justify-center rounded-xl border border-white/20 bg-white/5 px-5 py-2.5 text-sm font-semibold text-white/90 hover:bg-white/10">
            Back to Pickups
          </Link>
        </div>
      </form>
